    ...
    public static function forDropdown($business_id, $with_data = false, $q = '')
    {
        ...
        if ($with_data) {
            ...
            foreach ($accounts as $k => $v) {
                ...
                if (empty($v->sub_type)) {
    $accounts[$k]->sub_type = '';
} elseif (! empty($v->sub_type_business_id)) {
    $accounts[$k]->sub_type = $v->sub_type;
} else {
    $translation_key = 'accounting::lang.'.$v->sub_type;
    $translated = __($translation_key);

    $accounts[$k]->sub_type = $translated === $translation_key
        ? $v->sub_type
        : $translated;
}
            }
