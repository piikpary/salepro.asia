    ...
    public function index()
{
    ...
    // ── Zone-based access control (API) ───────────────────────────────────
    // Rule 1 : admin user  OR  zone_access_all = 1  →  see all contacts of business
    // Rule 2 : non-admin WITH location zones assigned  →  filter by those zones
    //          (contact must have a contacts_map row that matches at least one zone)
    //          + also include contacts directly assigned via user_contact_access
    // Rule 3 : non-admin WITHOUT any location zone  →  only contacts they created
    //          + also include contacts directly assigned via user_contact_access
    $isAdmin = ($user->user_type === 'admin') || ((int) $user->zone_access_all === 1);

    if (! $isAdmin) {
        $userZones = DB::table('user_location_zones')
            ->where('user_id', $user->id)
            ->get();

        // Sub-query: contacts directly assigned to this user via user_contact_access
        $assignedToUser = function ($sub) use ($user) {
            $sub->select(DB::raw(1))
        ...
        if ($userZones->isNotEmpty()) {
            // ── Rule 2: zone match  OR  directly assigned ──────────────────
            $query->where(function ($q) use ($userZones, $assignedToUser) {
                // Original zone filter
                $q->whereExists(function ($sub) use ($userZones) {
                    $sub->select(DB::raw(1))
                        ->from('contacts_map as cm_zone')
                        ->whereColumn('cm_zone.contact_id', 'contacts.id')
                        ->whereNull('cm_zone.deleted_at')
                        ->where(function ($zq) use ($userZones) {
                            foreach ($userZones as $zone) {
                                $zq->orWhere(function ($zoneClause) use ($zone) {
                                    // Province must always match (every zone has at least a province)
                                    $zoneClause->where('cm_zone.province_id', $zone->province_id);

                                    // Narrow to district only if the zone specifies one
                                    if (! is_null($zone->district_id)) {
                                        $zoneClause->where('cm_zone.district_id', $zone->district_id);
                                    }

                                    // Narrow to commune only if the zone specifies one
                                    if (! is_null($zone->commune_id)) {
                                        $zoneClause->where('cm_zone.commune_id', $zone->commune_id);
                                    }
                                });
                            }
                        });
                })
                // NEW: also include contacts directly assigned to this user
                ->orWhereExists($assignedToUser);
            });
        } else {
            // ── Rule 3: no zones → created_by  OR  directly assigned ───────
            $query->where(function ($q) use ($user, $assignedToUser) {
                $q->where('contacts.created_by', $user->id)
                  ->orWhereExists($assignedToUser);
            });
        }
    }
    // Rule 1 (admin / zone_access_all): no extra filter — all business contacts are returned
