    ...
    public function index()
{
    ...
    public function loggedin()
    {
        ...
        // ✅ FIX 1: safe check for driver_app key
        if (!empty($filter['driver_app'] ?? null) && ($filter['driver_app'] == 1)) {
        ...
        // ✅ FIX 2: extract exchange_rate value BEFORE touching business object
        //    Cannot do $user['business']['middle_currency']['exchange_rate'] = ...
        //    because App\Business is an Eloquent Model — indirect modification crashes
        $exchangeRate = $user['business']['middle_currency']['myCurrency']['exchange_rate'] ?? 0;

        $user['default_currency']             = $defaultCurrency ?? 4100;
        $user['middle_currency_exchange_rate'] = $exchangeRate;
        $user['footer_text']                  = $footerText ?? '';
        $user['app_updates']                  = $appUpdates ?? [];
        $user['profile_image']                = $profileImage ?? null;

        // ✅ FIX 3: removed unset($user['business']['middle_currency']['myCurrency'])
        //    — that also caused "Indirect modification" crash on Eloquent model
