...
class ProductionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (empty($ingredient_quantities) || !is_array($ingredient_quantities)) {
                DB::rollBack();

                return redirect()->back()->with([
                    'status' => [
                        'success' => 0,
                        'msg' => 'Ingredients are missing. Please select a product with a valid recipe before submitting.',
                    ],
                ]);
            }
            ...
            foreach ($all_variation_details as $variation_details) {
                ...
                $variationId = $variation_details['id'];

                if (empty($ingredient_quantities[$variationId]) || !is_array($ingredient_quantities[$variationId])) {
                    continue;
                }

                $ingredientLine = $ingredient_quantities[$variationId];

                $line_sub_unit_id = !empty($ingredientLine['sub_unit_id'])
                    ? $ingredientLine['sub_unit_id']
                    : null;

                $line_multiplier = 1;
                if (!empty($line_sub_unit_id) && !empty($variation_details['sub_units'][$line_sub_unit_id]['multiplier'])) {
                    $line_multiplier = $variation_details['sub_units'][$line_sub_unit_id]['multiplier'];
                }

                $mfg_waste_percent = !empty($ingredientLine['mfg_waste_percent'])
                    ? $this->productUtil->num_uf($ingredientLine['mfg_waste_percent'])
                    : 0;

                $mfg_ingredient_group_id = !empty($ingredientLine['mfg_ingredient_group_id'])
                    ? $ingredientLine['mfg_ingredient_group_id']
                    : null;

                $line_qty = !empty($ingredientLine['quantity'])
                    ? $this->productUtil->num_uf($ingredientLine['quantity'])
                    : 0;
            ...
            if (empty($sell_lines)) {
                ...
                return redirect()->back()->with([
                    'status' => [
                        'success' => 0,
                        'msg' => 'No valid ingredients were found for this production. Please reload the recipe and try again.',
                    ],
                ]);
            }
    ...
    public function edit($id)
    {
        ...
        return view('manufacturing::production.edit')->with(compact(
            'production_purchase',
            'production_sell',
            'business_locations',
            'recipe_dropdown',
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            foreach ($production_sell->sell_lines as $sell_line) {
                $variation = $sell_line->variations;
                $sellLineId = $sell_line->id;

                if (empty($ingredient_quantities[$sellLineId]) || !is_array($ingredient_quantities[$sellLineId])) {
                    continue;
                }

                $ingredientLine = $ingredient_quantities[$sellLineId];
                ...
                if (!empty($line_sub_unit_id)) {
                    ...
                    $line_multiplier = !empty($sub_units[$line_sub_unit_id]['multiplier'])
                        ? $sub_units[$line_sub_unit_id]['multiplier']
                        : 1;
                }
