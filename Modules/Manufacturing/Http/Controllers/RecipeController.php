    ...
    public function getIngredientRow($variation_id)
{
    ...
    if (!empty($sub_units) && is_array($sub_units)) {
        foreach ($sub_units as $key => $value) {
            if (is_array($value)) {
                $sub_units[$key]['multiplier'] = $value['multiplier'] ?? 1;
                $sub_units[$key]['name'] = $value['name'] ?? ($value['actual_name'] ?? $ingredient->product->unit->short_name);
                $sub_units[$key]['actual_name'] = $value['actual_name'] ?? $sub_units[$key]['name'];
            } else {
                $sub_units[$key] = [
                    'name' => $value ?: $ingredient->product->unit->short_name,
                    'actual_name' => $value ?: $ingredient->product->unit->short_name,
                    'multiplier' => 1,
                ];
            }
        }
    ...
    $ig_index = request()->input('row_ig_index', 0);
    $sort_order = request()->input('sort_order', 1);

    return view('manufacturing::recipe.ingredient_row', compact(
        'ingredient',
        'row_index',
        'ig_index',
        'sort_order'
    ...
    public function getRecipeDetails()
{
    ...
    if (!empty($recipe)) {
        ...
        foreach ($recipe->ingredients as $ingredient) {
            $ingredient_quantity = $this->mfgUtil->calc_percentage(
                $ingredient->quantity,
        ...
        if (!empty($ingredients) && is_array($ingredients)) {
            foreach ($ingredients as $iKey => $ingredient) {
                if (!array_key_exists('allow_decimal', $ingredients[$iKey])) {
                    $ingredients[$iKey]['allow_decimal'] = 1;
                }

                if (!array_key_exists('multiplier', $ingredients[$iKey]) || empty($ingredients[$iKey]['multiplier'])) {
                    $ingredients[$iKey]['multiplier'] = 1;
                }

                if (!empty($ingredients[$iKey]['sub_units']) && is_array($ingredients[$iKey]['sub_units'])) {
                    foreach ($ingredients[$iKey]['sub_units'] as $sKey => $sValue) {
                        if (is_array($sValue)) {
                            $ingredients[$iKey]['sub_units'][$sKey]['multiplier'] = $sValue['multiplier'] ?? 1;
                            $ingredients[$iKey]['sub_units'][$sKey]['name'] = $sValue['name'] ?? ($sValue['actual_name'] ?? '');
                            $ingredients[$iKey]['sub_units'][$sKey]['actual_name'] = $sValue['actual_name'] ?? $ingredients[$iKey]['sub_units'][$sKey]['name'];
                            $ingredients[$iKey]['sub_units'][$sKey]['allow_decimal'] = $sValue['allow_decimal'] ?? 1;
                        } else {
                            $ingredients[$iKey]['sub_units'][$sKey] = [
                                'multiplier' => 1,
                                'name' => $sValue,
                                'actual_name' => $sValue,
                                'allow_decimal' => 1,
    ...
    $ingredient_table = view(
        'manufacturing::recipe.ingredients_for_production',
        compact('ingredients', 'recipe', 'pos_settings', 'manufacturing_settings')
    )->render();
    ...
    if (!empty($recipe) && !empty($recipe->variation) && !empty($recipe->variation->product) && !empty($recipe->variation->product->unit)) {
        ...
        if (!empty($sub_units) && is_array($sub_units)) {
            ...
            foreach ($sub_units as $key => $value) {
                $multiplier = is_array($value) ? ($value['multiplier'] ?? 1) : 1;
                $name = is_array($value) ? ($value['name'] ?? ($value['actual_name'] ?? $unit_name)) : $value;

                $unit_html .= '<option value="' . $key . '" data-multiplier="' . $multiplier . '" data-unit_name="' . $name . '" ';
