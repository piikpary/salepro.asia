<tr>
    <td>
        ...
        <input type="hidden" name="ingredients[{{ $row_index }}][sort_order]" class="sort_order" value="@if(!empty($ingredient->sort_order)){{ $ingredient->sort_order }}@elseif(!empty($sort_order)){{ $sort_order }}@endif">
    ...
    <td>
        <div class="input-group">
            {!! Form::text(
                'ingredients[' . $row_index . '][waste_percent]',
                !empty($ingredient->waste_percent) ? @num_format($ingredient->waste_percent) : 0,
                [
                    'class' => 'form-control input_number waste_percent input-sm',
                    'placeholder' => __('lang_v1.waste_percent')
                ]
            ) !!}
    ...
    <td>
        <div class="@if(empty($ingredient->sub_units)) input-group @else input_inline @endif">
            {!! Form::text(
                'ingredients[' . $row_index . '][quantity]',
                !empty($ingredient->quantity) ? @format_quantity($ingredient->quantity) : 1,
                [
            ...
            <span class="@if(empty($ingredient->sub_units)) input-group-addon @endif">
                @if(!empty($ingredient->sub_units) && is_array($ingredient->sub_units))
    ...
    @php
        $basePrice = !empty($ingredient->quantity)
            ? ((float) $ingredient->quantity * (float) ($ingredient->dpp_inc_tax ?? 0))
            : (float) ($ingredient->dpp_inc_tax ?? 0);

        $rowMultiplier = !empty($ingredient->multiplier) ? (float) $ingredient->multiplier : 1;
        $price = $basePrice * $rowMultiplier;
    @endphp

    <td>
        <span class="ingredient_price">{{ @num_format($price) }}</span>
    </td>

    <td>
        <button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-error remove_ingredient">
