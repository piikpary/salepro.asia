    ...
    private function getDataForTableCustomerRing(Request $request)
    {
        ...
        $query = DB::table('contacts')
            ...
            ->select(
                ...
                DB::raw("(SELECT SUM(srbc.stock_ring_balance) FROM stock_ring_balance_customer srbc INNER JOIN products p ON p.id = srbc.product_id WHERE srbc.contact_id = contacts.id AND srbc.business_id = {$business_id}) as total_ring_balance_sum")
            );
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $summedData = $data->groupBy('contact_id')->map(function ($items) {
                ...
                $firstItem->total_ring_balance = $firstItem->total_ring_balance_sum ?? 0;
