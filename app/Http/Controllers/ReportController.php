...
public function SaleRevenueAR(Request $request)
{
    ...
    try {
        // FIX: append '-01' to force day=1, preventing Carbon overflow
        // e.g. on March 31, createFromFormat('Y-m','2026-02') would overflow to March 3
        $toDate   = \Carbon\Carbon::createFromFormat('Y-m-d', $toMonthParam   . '-01');
        $fromDate = \Carbon\Carbon::createFromFormat('Y-m-d', $fromMonthParam . '-01');
    } catch (\Exception $e) {
    ...
    foreach ($monthRange as $monthParam) {
        ...
        // FIX: append '-01' to prevent Carbon day-overflow (e.g. Feb on day 31)
        $refDate = \Carbon\Carbon::createFromFormat('Y-m-d', $monthParam . '-01');
        ...
        // FIX: strip any month newer than the current box's month
        // (e.g. March's outstanding A/R must not bleed into the February box)
        $allMonths = array_values(array_filter($allMonths, function ($m) use ($currentKey) {
            return $m <= $currentKey;
        }));
        ...
        foreach ($allMonths as $key) {
            // FIX: append '-01' to prevent Carbon day-overflow
            $mDate  = \Carbon\Carbon::createFromFormat('Y-m-d', $key . '-01');
        ...
        // -------------------------------------------------------
        // 3. Totals
        //    Total Revenue = ALL payments collected in the current month
        //                    Computed directly (matches Daily Payment Report exactly)
        //                    — no deleted_at filter, same as Daily Payment Report
        //    Total A/R     = sum of all currently outstanding balances
        // -------------------------------------------------------
        $totalRevenue = (float) DB::table('transaction_payments as tp')
            ->join('transactions as t', 'tp.transaction_id', '=', 't.id')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->where('t.status', 'final')
...
public function ClaimReport(Request $request)
{
    ...
    public function DailySaleSummary(Request $request)
    {
        ...
        $currentDay = $today->format('D'); // "Mon", "Sat" …

        // Find the schedule with back_days configured for daily_sale_summary
        $schedule = DB::table('telegram_schedules')
            ->where('business_id', $business_id)
            ->where('is_active', 1)
            ->where(function ($q) {
                $q->whereNull('report_types')
                  ->orWhereRaw('JSON_CONTAINS(report_types, \'"daily_sale_summary"\')');
            })
            ->whereNotNull('back_days')
            ->first();

        $backDaysMap = json_decode($schedule->back_days ?? '{}', true) ?? [];
        $backDays    = max(1, (int) ($backDaysMap[$currentDay] ?? 1));

        // Build one report array per date (today-1 … today-backDays)
        $reports = [];
        for ($i = 1; $i <= $backDays; $i++) {
            $date      = $today->copy()->subDays($i)->toDateString();
            $reports[] = array_merge(['date' => $date], $this->buildDailySaleSummaryData($business_id, $date));
        }

        // Date chip label: "29-May-2026 & 28-May-2026"
        $dateChip = implode(' & ', array_map(
            fn($r) => \Carbon\Carbon::parse($r['date'])->format('d-M-Y'),
            $reports
        ));

        return view('report.daily_sale_summary', compact('business', 'reports', 'dateChip'));
    }

    private function buildDailySaleSummaryData(int $business_id, string $date): array
    {
        ...
        $payment_method_labels = $this->transactionUtil->payment_types(null, false, $business_id);
        ...
        // ── Section 3: Outstanding AR Aging Summary ──
        ...
        $today_obj = \Carbon\Carbon::parse($date);
        $aging = [
        ...
        foreach ($outstanding_raw as $row) {
            $days      = \Carbon\Carbon::parse($row->t_date)->diffInDays($today_obj);
        ...
        // ── Section 4: Daily Reward Exchange ──
        ...
        if (!empty($reward_pfs_ids)) {
            ...
            // Build prize groups: one row per receive_product (prize), joining prize name
            ...
            foreach ($prize_rows as $row) {
                $prize_id = $row->receive_product;
                if (!isset($prizes[$prize_id])) {
                    $prizes[$prize_id] = [
                ...
                $prizes[$prize_id]['pfs_ids'][] = $row->product_for_sale;
            }
            ...
            foreach ($prizes as $prize) {
                ...
                $outstanding_rings = max(0, $receivable - $received_rings);
    ...
    public function sendDailySaleSummaryToTelegram(Request $request)
    {
        try {
            $business_id = $request->session()->get('user.business_id');
            ...
            if ($schedules->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => 'No active Telegram setup found for Daily Sale Summary report.',
                ]);
            }

            // Date passed from JS (one AJAX call per date section screenshot)
            $dateLabel = $request->filled('date')
                ? \Carbon\Carbon::parse($request->input('date'))->format('d-M-Y')
                : \Carbon\Carbon::yesterday()->format('d-M-Y');

            $image_data   = $request->input('image');
            $image_parts  = explode(';base64,', $image_data);
            $image_base64 = base64_decode($image_parts[1]);
            $fileName     = 'dss_manual_' . time() . '.png';
            ...
            // Deduplicate by chat_id: one send per unique chat_id.
            $byChat = [];
            foreach ($schedules as $s) {
                if (empty($s->chat_id)) continue;
                $cid = $s->chat_id;
                if (!isset($byChat[$cid]) || (!empty($s->back_days) && empty($byChat[$cid]->back_days))) {
                    $byChat[$cid] = $s;
                }
            }

            foreach ($byChat as $schedule) {
    ...
    private function buildPaymentMethodLabels($business_id, $location_id)
    {
        $labels = [];
        if (!$location_id) return $labels;
        $location = BusinessLocation::find($location_id);
        if (!$location || !$location->default_payment_accounts) return $labels;

        $accounts = json_decode($location->default_payment_accounts, true) ?? [];
        $business = Business::find($business_id);
        $custom_pay_labels = [];
        if ($business && $business->custom_labels) {
            $parsed = json_decode($business->custom_labels, true) ?? [];
            $custom_pay_labels = $parsed['payments'] ?? [];
        }
        foreach ($accounts as $method => $cfg) {
            if (empty($cfg['is_enabled']) || $cfg['is_enabled'] != '1') continue;
            if (strpos($method, 'custom_pay_') === 0) {
                $labels[$method] = $custom_pay_labels[$method] ?? ucfirst(str_replace('_', ' ', $method));
            } elseif ($method === 'cash_ring_percentage') {
                $labels[$method] = 'Cash ring(%)';
            } else {
