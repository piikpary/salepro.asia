 ...
 public function index(Request $request)
{
    if ($request->ajax()) {
        ...
        $relatedRewards = RewardsExchange::whereIn('product_for_sale', $pageProductIds)
            ...
            ->where('rewards_exchange.business_id', $business_id)  // FIX: scope to current business — prevents wrong exchange_product_id from other businesses causing bad merge
            ->where('rewards_exchange.type', 'customers')           // FIX: only customer-type rewards
            ->whereNull('rewards_exchange.deleted_at')
            ->select([
                'rewards_exchange.product_for_sale as product_for_sale_id',
                'rewards_exchange.exchange_product as exchange_product_id',  // used as merge key for Ring Received rowspan
        ...
        foreach ($pagedTrans as $transaction) {
                ...
                foreach ($transaction->sell_lines as $sellLine) {
                    ...
                    foreach ($rewards as $reward) {
                        ...
                        $rowsData[] = [
                            ...
                            'exchange_product_id' => $reward->exchange_product_id,
                        ];
                    }
                }
            }
            ...
            // FIX: Sort $rowsData by exchange_product_id so that rows sharing the same ring
            // are always adjacent. HTML rowspan only works for contiguous rows — if coca2 exchange
            // and Vida Prize both map to exchange_product_id=10262 but Cofee Prize sits between
            // them, the rowspan cell breaks visually. Sorting fixes this.
            usort($rowsData, function($a, $b) {
                $eidA = $a['exchange_product_id'] ?? PHP_INT_MAX;
                $eidB = $b['exchange_product_id'] ?? PHP_INT_MAX;
                if ($eidA === $eidB) return 0;
                return ($eidA < $eidB) ? -1 : 1;
            });

            // Post-process rowspan: rows sharing the same exchange_product_id are now adjacent,
            // so only the first row for each ring shows ring_received; the rest get rowspan=0.
            $seenExchangeIds = [];
            foreach ($rowsData as $idx => &$row) {
                $eid = $row['exchange_product_id'] ?? null;
                if ($eid !== null) {
                    if (!isset($seenExchangeIds[$eid])) {
                        $seenExchangeIds[$eid] = $idx;
                        $row['ring_received_rowspan'] = 1;
                    } else {
                        $firstIdx = $seenExchangeIds[$eid];
                        $rowsData[$firstIdx]['ring_received_rowspan']++;
                        $row['ring_received'] = '-';
                        $row['ring_received_rowspan'] = 0;
                    }
                } else {
                    $row['ring_received_rowspan'] = 1;
                }
            ...
            foreach ($rowsData as $idx => $row) {
                $result[] = (object) [
                    ...
                    'ring_received_rowspan' => $row['ring_received_rowspan'] ?? 1,
