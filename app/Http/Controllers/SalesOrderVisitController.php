...
private function applyFilters($query, Request $request)
{
    $filters = [
        ...
        'customer_group_id' => 'contacts.customer_group_id',
    ];
...
public function index(Request $request)
{
    if ($request->ajax()) {
        ...
        return DataTables::of($query)
            ...
            ->editColumn('sale_rep', '{{ $sale_rep ?? "" }}')
            ->editColumn('customer_group_name', '{{ $customer_group_name ?? "" }}')
            ...
            ->rawColumns(['action', 'own_product', 'other_product', 'checkin_distance', 'customer_group_name'])
            ->make(true);
    }
    ...
    $customer_groups = Cache::remember("customer_groups_{$business_id}", 3600, function () use ($business_id) {
        return CustomerGroup::where('business_id', $business_id)
            ->pluck('name', 'id')
            ->prepend(__('lang_v1.all'), '');
    });
    ...
    public function show($id)
    {
        ...
        // ── Separate visited own vs other lines ──────────────────────────────
        $visitedOwn   = [];   // keyed by product_id
        $visitedOther = [];   // keyed by product_id

        foreach ($visit->TransactionSellLineVisit as $line) {
            if (!$line->product) continue;
            if ($line->kind_product == 0) {
                $visitedOwn[$line->product_id] = $line;
            } else {
                $visitedOther[$line->product_id] = $line;
            }
        }

        // ── Load competitor mappings for every visited own product ────────────
        $ownProductData = \App\Product::whereIn('id', array_keys($visitedOwn))
            ->select(['id', 'name', 'assigned_competitors_product'])
            ->get()
            ->keyBy('id');

        // ── Build grouped sections ────────────────────────────────────────────
        // $groups = [ [ 'own_line' => $line, 'competitors' => [$line, ...] ], ... ]
        $groups           = [];
        $claimedOtherIds  = [];

        foreach ($visitedOwn as $ownId => $ownLine) {
            $linkedIds = json_decode(
                optional($ownProductData->get($ownId))->assigned_competitors_product ?? '[]',
                true
            ) ?? [];

            $competitors = [];
            foreach ($linkedIds as $compId) {
                if (isset($visitedOther[$compId])) {
                    $competitors[]     = $visitedOther[$compId];
                    $claimedOtherIds[] = $compId;
                }
            }
        ...
        // ── Other / Unmapped: other products not claimed by any visited own ───
        $unmapped = [];
        foreach ($visitedOther as $otherId => $otherLine) {
            if (!in_array($otherId, $claimedOtherIds)) {
                $unmapped[] = $otherLine;
            }
        }
        ...
        if ($visit->TransactionSellLineVisitImage) {
            foreach ($visit->TransactionSellLineVisitImage as $image) {
                if (isset($image->image)) {
                    $images[] = "https://piik-data.sgp1.digitaloceanspaces.com/piik-data/sell-visit/images/" . $image->image;
                }
            }
        }

        return view('sales_visit.show', compact('visit', 'groups', 'unmapped', 'images'));
    }
    ...
    public function getVisitHistory(Request $request)
    {
        ...
        foreach ($visits as $visit) {
            ...
            $points = optional(optional($visit->contact)->contactMaps)->points;
