...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'money_reward' => ! empty($input['money_reward']) ? $input['money_reward'] : null,
            'money_percentage' => (isset($input['money_percentage']) && $input['money_percentage'] !== '') ? ($uf_data ? $this->num_uf($input['money_percentage']) : $input['money_percentage']) : null,
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type, $business_id)
    {
        ...
        $output = [
            ...
            'table_index_label' => !empty($il->table_index_label) ? $il->table_index_label : null,
        ];
        ...
        if (! empty($transaction->sales_order_ids)) {
            $so_ids = is_array($transaction->sales_order_ids)
                ? $transaction->sales_order_ids
                : json_decode($transaction->sales_order_ids, true);

            $sale_orders = ! empty($so_ids)
                ? Transaction::where('type', 'sales_order')->whereIn('id', $so_ids)->get()
                : collect();

            if ($sale_orders && $sale_orders->isNotEmpty()) {
                $output['sale_orders_invoice_no'] = implode(', ', $sale_orders->pluck('invoice_no')->toArray());
                $sale_orders_invoice_date = [];
                foreach ($sale_orders as $sale_order) {
                    if (blank($il->date_time_format ?? null)) {
        ...
        // Sales order reward fields
        $output['transaction_type'] = $transaction_type;
        $output['money_reward']     = $transaction->money_reward;
        $output['money_percentage'] = $transaction->money_percentage;
        $output['money_percentage_amount'] = null;
        $output['amount_to_pay']           = null;

        // Only calculate for sales_order with a reward value
        if ($transaction_type == 'sales_order' && !empty($transaction->money_reward)) {
            $reward     = (float) $transaction->money_reward;
            $percentage = (float) ($transaction->money_percentage ?? 0);
            $pct_amount = $percentage / 100 * $reward;   // e.g. 11% of 100 = 11

            // ភាគរយ shows the calculated dollar amount (not the % itself)
            $output['money_percentage_amount'] = $this->num_f($pct_amount, $show_currency, $business_details);

            // លុយត្រូវទូទាត់ = total - money_reward - (money_percentage% of money_reward)
            // e.g. 200 - 100 - 11 = 89
            $output['amount_to_pay'] = $this->num_f(
                max(0, $transaction->final_total - $reward - $pct_amount),
                $show_currency,
                $business_details
            );
        }
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if ($transaction->status == 'draft') {
            ...
            // Soft-delete payments for sell (draft): keep rows as history
            if ($transaction->type == 'sell' && $transaction_payments->isNotEmpty()) {
                $this->softDeleteTransactionPayments($transaction_payments);
            ...
            // Soft-delete payments for sell: set deleted_at/deleted_by, keep rows as history
            if ($transaction->type == 'sell' && $transaction_payments->isNotEmpty()) {
                $this->softDeleteTransactionPayments($transaction_payments);
            }
            ...
            // Fire payment deleted events (only for sell) — triggers DeleteAccountTransaction listener
            if ($transaction->type == 'sell') {
                foreach ($transaction_payments as $payment) {
                    event(new TransactionPaymentDeleted($payment));
                }
    ...
    /**
     * Soft-delete all payment lines for a sell transaction being deleted.
     * Sets deleted_at + deleted_by on each row — does NOT remove from DB (keeps history).
     * Also handles parent (advance/split) payments: reduces their amount or soft-deletes them.
     */
    private function softDeleteTransactionPayments($transaction_payments)
    {
        $now        = \Carbon\Carbon::now();
        $deleted_by = auth()->id();

        // Aggregate child-payment amounts per parent to avoid double-updating
        $parent_adjustments = [];
        foreach ($transaction_payments as $payment) {
            if (!empty($payment->parent_id)) {
                $parent_adjustments[$payment->parent_id] =
                    ($parent_adjustments[$payment->parent_id] ?? 0) + $payment->amount;
            }
        }

        // Soft-delete parent payments or reduce their balance
        foreach ($parent_adjustments as $parent_id => $total_amount) {
            $parent = TransactionPayment::find($parent_id);
            if (!$parent) {
                continue;
            }
            $parent->amount = max(0, $parent->amount - $total_amount);
            if ($parent->amount <= 0) {
                $parent->deleted_at = $now;
                $parent->deleted_by = $deleted_by;
                $parent->save();
                event(new TransactionPaymentDeleted($parent));
            } else {
                $parent->save();
                event(new TransactionPaymentUpdated($parent, null));
            }
        }

        // Soft-delete all payment rows for this transaction (keep as history)
        foreach ($transaction_payments as $payment) {
