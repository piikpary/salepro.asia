        ...
        <form method="POST" action="{{ route('login') }}" id="login-form">
            ...
            @if (session('status'))
                    @php
                        ...
                        $success = is_array($status) ? ($status['success'] ?? 0) : 0;
                    @endphp

                    @if (!empty($message))
                        <div class="alert {{ $success ? 'alert-success' : 'alert-danger' }}" style="margin-bottom: 15px;">
                            {{ $message }}
                        </div>
                    @endif
                @endif
                ...
                @if (session('login_lockout_seconds'))
                    <div class="alert alert-warning" style="margin-bottom: 15px;">
                        Please wait
                        <strong id="login-countdown">{{ session('login_lockout_seconds') }}</strong>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        if (countdownEl) {
            var seconds = parseInt(countdownEl.innerText, 10);

            // Disable login form during countdown
            $('#username').prop('readonly', true);
            $('#password').prop('readonly', true);
            $('input[name="remember"]').prop('disabled', true);
            $('.btn-login')
                .prop('disabled', true)
                .text('Please wait ' + seconds + 's');
            ...
            var timer = setInterval(function () {
                ...
                if (seconds <= 0) {
                    ...
                    $('input[name="remember"]').prop('disabled', false);
                    $('.btn-login')
                        .prop('disabled', false)
                        .text("@lang('lang_v1.login')");
                ...
                $('.btn-login').text('Please wait ' + seconds + 's');
            }, 1000);
        }
