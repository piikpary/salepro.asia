                ...
                <tbody>
                    ...
                    @foreach($arLabels as $rowKey => $rowLabel)
                        @php
                            ...
                            $mDate       = \Carbon\Carbon::createFromFormat('Y-m-d', $rowKey . '-01');
                                ...
                                @foreach($arLabels as $colKey => $colLabel)
                                    ...
                                    @if($showCol)
                                        @if($colKey === $rowKey)
                                            ...
                                            @php
                                                // FIX: append '-01' to prevent Carbon day-overflow
                                                $arMonthDate = \Carbon\Carbon::createFromFormat('Y-m-d', $colKey . '-01');
                    ...
                    <tr class="footer-ar">
                        ...
                        @foreach($arLabels as $key => $label)
                            ...
                            @if($showCol)
                                @php
                                    // FIX: append '-01' to prevent Carbon day-overflow
                                    $arMonthDate = \Carbon\Carbon::createFromFormat('Y-m-d', $key . '-01');
                                    $arStartDate = $arMonthDate->copy()->startOfMonth()->format('Y-m-d');
                                    $arEndDate   = $arMonthDate->copy()->endOfMonth()->format('Y-m-d');
                                    $arUrl = action([\App\Http\Controllers\SellController::class, 'index'])
                                           . '?ar_month=' . $key
