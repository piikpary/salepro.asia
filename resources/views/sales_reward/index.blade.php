...
$(document).ready(function() {
    ...
    function initializeDataTable() {
        ...
        rewardTable = $('#sale_order_reward').DataTable({
            ...
            drawCallback: function(settings) {
                ...
                // Merge Ring Received (col 12) for rows sharing the same exchange product
                data.each(function(rowData, i) {
                    if (rowData.ring_received_rowspan > 1) {
                        var cell = $(rows[i]).find('td').eq(12);
                        if (cell.length > 0) {
                            cell.attr('rowspan', rowData.ring_received_rowspan).addClass('merged-cell');
                            for (var j = 1; j < rowData.ring_received_rowspan; j++) {
                                if (i + j < rows.length) {
                                    $(rows[i + j]).find('td').eq(12).hide();
                                }
                            }
                        }
                    }
    ...
    function processDataForExport(data) {
        ...
        data.forEach(function(row) {
            ...
            // Only show ring_received for the first row of each exchange product group
            var ringReceived = (row.ring_received_rowspan === 0) ? '' : (row.ring_received || '');
    ...
    function exportToExcel() {
        ...
        getAllDataForExport(function(data) {
            ...
            // Merge Ring Received (column 11, 0-indexed) for duplicate exchange product rows
            for (var k = 0; k < data.length; k++) {
                if (data[k].ring_received_rowspan > 1) {
                    var rs = data[k].ring_received_rowspan;
                    merges.push({ s: {r: k + 1, c: 11}, e: {r: k + rs, c: 11} });
                }
            }
