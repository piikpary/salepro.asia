...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
	...
	@if(!empty($delivery_note_id))
		...
		<input type="hidden" id="auto_load_dn_id" value="{{ $delivery_note_id }}">
	@endif
	<div class="row">
		<div class="col-md-12 col-sm-12">
			@component('components.widget', ['class' => 'box-solid'])
				...
				@if(!empty($status) && $status == 'proforma')
					{{-- Proforma: show dropdown pre-selected so user can see/change status --}}
					<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
						<div class="form-group">
							{!! Form::label('status', __('sale.status') . ':*') !!}
							{!! Form::select('status', $statuses, 'proforma', ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']); !!}
						</div>
					</div>
				...
				<div class="col-sm-3">
					...
					{!! Form::select('commission_agent',
								$commission_agent, $selected_commission_agent ?? null, ['class' => 'form-control select2', 'id' => 'commission_agent', 'required' => $is_commission_agent_required]); !!}
					</div>
				</div>
				@endif
		        <div class="col-sm-3">
...
@endcomponent
			...
			@component('components.widget', ['class' => 'box-solid'])
			    ...
			    <div class="col-md-6">
			    	<div class="form-group">
						{!! Form::label('money_reward', 'Reward Note') !!}
						{!! Form::textarea('money_reward', !empty($edit) ? $transaction->money_reward : null, ['class' => 'form-control', 'rows' => 1, 'placeholder' => 'Reward Note', 'style' => 'resize:none; height:38px;']); !!}
					</div>
			    </div>
			    <div class="col-md-6">
			    	<div class="form-group">
						{!! Form::label('money_percentage', 'Reward Percentage (%)') !!}
						{!! Form::text('money_percentage', !empty($edit) ? $transaction->money_percentage : null, ['class' => 'form-control', 'placeholder' => '0.00', 'inputmode' => 'decimal', 'style' => 'height:38px;']); !!}
					</div>
			    </div>
			    @endif
	...
	@endphp
	@if((empty($status) || (!in_array($status, ['quotation', 'draft', 'proforma'])) || $is_enabled_download_pdf) && $sale_type != 'sales_order')
		@can('sell.payments')
