...
use App\Http\Controllers\SmartCallPlanController;
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin','throttle:120,1'])->group(function () {
    ...
    Route::get('/sell/create-proforma-from-sales-order/{id}', [SellController::class, 'createProformaFromSalesOrder'])->name('sell.createProformaFromSalesOrder');
    ...
    Route::get('/reports/daily-sale-summary', [ReportController::class, 'DailySaleSummary'])->name('reports.daily-sale-summary');
    Route::post('/reports/daily-sale-summary/send-telegram', [ReportController::class, 'sendDailySaleSummaryToTelegram'])->name('reports.daily-sale-summary.send-telegram');
    ...
    // Product Sale Targets
    Route::get('product-sale-targets/assigned-list', [\App\Http\Controllers\ProductSaleTargetController::class, 'assignedList'])->name('product-sale-targets.assigned-list');
    Route::post('product-sale-targets/generate-grid', [\App\Http\Controllers\ProductSaleTargetController::class, 'generateGrid'])->name('product-sale-targets.generate-grid');
    Route::get('product-sale-targets/search-products', [\App\Http\Controllers\ProductSaleTargetController::class, 'searchProducts'])->name('product-sale-targets.search-products');
    Route::delete('product-sale-targets/detail/{id}', [\App\Http\Controllers\ProductSaleTargetController::class, 'destroyDetail'])->name('product-sale-targets.destroy-detail');
    Route::get('product-sale-targets/import-modal', [\App\Http\Controllers\ProductSaleTargetController::class, 'importModal'])->name('product-sale-targets.import-modal');
    Route::get('product-sale-targets/download-template', [\App\Http\Controllers\ProductSaleTargetController::class, 'downloadTemplate'])->name('product-sale-targets.download-template');
    Route::post('product-sale-targets/import', [\App\Http\Controllers\ProductSaleTargetController::class, 'import'])->name('product-sale-targets.import');
    Route::resource('product-sale-targets', \App\Http\Controllers\ProductSaleTargetController::class);
    ...
    Route::post('ai-chat/new', [\App\Http\Controllers\AiChatController::class, 'newChat'])->name('ai-chat.new');
    Route::get('ai-chat/load/{session_id}', [\App\Http\Controllers\AiChatController::class, 'loadSession'])->name('ai-chat.load');
...
Route::get('sells/cache-stats', [SellController::class, 'getCacheStats'])->name('sells.cache-stats');
    ...
    Route::get('product_sale_visit/other-products', [ProductSaleVisitController::class, 'getOtherProducts'])->name('product_sale_visit.other_products');
    Route::post('product_sale_visit/toggle-visibility', [ProductSaleVisitController::class, 'toggleVisibility'])->name('product_sale_visit.toggle_visibility');
    Route::post('product_sale_visit/bind-competitors', [ProductSaleVisitController::class, 'bindCompetitors'])->name('product_sale_visit.bind_competitors');
    Route::get('product_sale_visit/available-competitors', [ProductSaleVisitController::class, 'getAvailableCompetitors'])->name('product_sale_visit.available_competitors');
    ...
    // -- Smart Call Plan / Visit Plan routes --------------
Route::get('/smart-call-plans', [SmartCallPlanController::class, 'index'])
    ->name('smart-call-plans.index');

Route::post('/smart-call-plans/generate-board', [SmartCallPlanController::class, 'generateBoard'])
    ->name('smart-call-plans.generate-board');

Route::post('/smart-call-plans/manual-draft', [SmartCallPlanController::class, 'manualDraft'])
    ->name('smart-call-plans.manual-draft');

Route::post('/smart-call-plans/manual-apply', [SmartCallPlanController::class, 'manualApply'])
    ->name('smart-call-plans.manual-apply');

Route::post('/smart-call-plans/task/{task}/log', [SmartCallPlanController::class, 'logTask'])
    ->name('smart-call-plans.task.log');

Route::post('/smart-call-plans/task/{task}/skip', [SmartCallPlanController::class, 'skipTask'])
    ->name('smart-call-plans.task.skip');

Route::get('/smart-call-plans/search-customers', [SmartCallPlanController::class, 'searchCustomers'])
    ->name('smart-call-plans.search-customers');

Route::post('/smart-call-plans/task/{task}/move', [SmartCallPlanController::class, 'moveTask'])
    ->name('smart-call-plans.task.move');


// -- Wildcard route LAST ------------------------------
Route::get('/call-plans/{id}', [AiCallPlanController::class, 'show'])
    ->name('call-plans.show');


// -- Import routes -----------------------------------
Route::get('/call-plans-import', [CallPlanImportController::class, 'form'])
    ->name('call-plans.import.form');

Route::post('/call-plans-import/customers', [CallPlanImportController::class, 'importCustomers'])
    ->name('call-plans.import.customers');

Route::post('/call-plans-import/orders', [CallPlanImportController::class, 'importOrders'])
    ->name('call-plans.import.orders');

Route::post('/call-plans-import/logs', [CallPlanImportController::class, 'importLogs'])
    ->name('call-plans.import.logs');

Route::post('/smart-call-plans/ai-recommendation', [SmartCallPlanController::class, 'aiRecommendation'])
    ->name('smart-call-plans.ai-recommendation');
    
Route::post('/smart-call-plans/draft-ai-reasons', [SmartCallPlanController::class, 'draftAiReasons'])
    ->name('smart-call-plans.draft-ai-reasons');
    ...
    try {
        ...
        ->post(config('services.anthropic.url', 'https://api.anthropic.com/v1/messages'), [
            'model' => config('services.anthropic.model'),
            'max_tokens' => 100,
            'messages' => [
                [
                    'role' => 'user',
                    'content' => 'Say only: API working',
                ],
            ],
        ]);
